...
fn class_manifest(diagram DiagramClassDiagram) []DiagramManifestEntry {
	...
	for i, class_def in diagram.classes {
		out << DiagramManifestEntry{
			id: diagram_manifest_id('class', '${class_def.name}|${class_def.members.join(",")}')
			kind: 'class'
			path: 'classes[${i}]'
...
fn er_manifest(diagram DiagramERDiagram) []DiagramManifestEntry {
	...
	for i, entity in diagram.entities {
		out << DiagramManifestEntry{
			id: diagram_manifest_id('entity', '${entity.name}|${entity.attributes.join(",")}')
			kind: 'entity'
			path: 'entities[${i}]'
...
fn git_manifest(graph DiagramGitGraph) []DiagramManifestEntry {
	...
	for i, event in graph.events {
		out << DiagramManifestEntry{
			id: diagram_manifest_id('git_event', '${event.kind}|${event.name}|${event.target}')
			kind: 'git_event'
			path: 'events[${i}]'
...
fn diagram_change_detail(previous DiagramDiffEntry, current DiagramDiffEntry) string {
	return match previous.kind {
		'graph_node' { graph_node_change_detail(previous.signature, current.signature) }
		'graph_edge' { graph_edge_change_detail(previous.signature, current.signature) }
		'timeline_entry' { timeline_entry_change_detail(previous.signature, current.signature) }
		'pipeline_stage' { pipeline_stage_change_detail(previous.signature, current.signature) }
		'state_transition' { state_transition_change_detail(previous.signature, current.signature) }
		'sequence_participant' { scalar_change_detail('name', previous.signature, current.signature) }
		'sequence_message' { sequence_message_change_detail(previous.signature, current.signature) }
		'sequence_note' { sequence_note_change_detail(previous.signature, current.signature) }
		'sequence_activation' { sequence_activation_change_detail(previous.signature, current.signature) }
		'sequence_block' { sequence_block_change_detail(previous.signature, current.signature) }
		'journey_section' { scalar_change_detail('title', previous.signature, current.signature) }
		'journey_step' { journey_step_change_detail(previous.signature, current.signature) }
		'class' { class_change_detail(previous.signature, current.signature) }
		'class_relation' { class_relation_change_detail(previous.signature, current.signature) }
		'entity' { entity_change_detail(previous.signature, current.signature) }
		'entity_relation' { entity_relation_change_detail(previous.signature, current.signature) }
		'gantt_section' { scalar_change_detail('title', previous.signature, current.signature) }
		'gantt_task' { gantt_task_change_detail(previous.signature, current.signature) }
		'git_event' { git_event_change_detail(previous.signature, current.signature) }
		'org_node' { org_node_change_detail(previous.signature, current.signature) }
		'tree_node' { scalar_change_detail('label', previous.signature, current.signature) }
		'graph_group' { graph_group_change_detail(previous.signature, current.signature) }
		else { '' }
	}
}
