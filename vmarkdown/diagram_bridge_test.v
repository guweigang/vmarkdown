...
fn test_render_flow_diagram_payload_uses_direction_and_labels() {
	payload := DiagramGraph{
		...
		nodes: [
			...
			DiagramNode{id: 'B', label: 'Go', shape: .round},
		]
		edges: [
...
fn test_render_sequence_diagram_payload() {
	payload := DiagramSequence{
		...
		events: [
			DiagramSequenceBlockBoundary{kind: .alt, label: 'success', start: true},
			DiagramSequenceMessage{from: 'Alice', to: 'Bob', text: 'ok', kind: .arrow},
			DiagramSequenceBlockBoundary{kind: .else_branch, label: 'miss', start: true},
			DiagramSequenceMessage{from: 'Bob', to: 'Bob', text: 'cache', kind: .arrow},
			DiagramSequenceBlockBoundary{kind: .alt, label: '', start: false},
		]
	}
...
fn test_render_class_diagram_payload() {
	payload := DiagramClassDiagram{
		classes: [
			DiagramClass{name: 'Animal', members: ['+name string', '+speak()']},
			DiagramClass{name: 'Dog'},
		]
		relations: [
			DiagramClassRelation{left: 'Animal', right: 'Dog', kind: '<|--', label: 'inherits'},
		]
	}
	rendered := render_diagram_payload(payload, 80)
...
fn test_render_er_diagram_payload() {
	payload := DiagramERDiagram{
		entities: [
			DiagramEntity{name: 'USER', attributes: ['string id']},
			DiagramEntity{name: 'ORDER', attributes: ['string id']},
		]
		relations: [
	...
	payload := DiagramGantt{
		...
		sections: [
			DiagramGanttSection{
				title: 'Build'
				tasks: [
					DiagramGanttTask{title: 'Compile', state: 'done', metadata: ['a1', '1d']},
				]
			},
		]
	}
