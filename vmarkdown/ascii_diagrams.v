...
fn render_ascii_grouped_flow_graph(graph DiagramGraph, width int) ?string {
	...
	if crossing_edges.len == 1 {
		...
		return if from_in_group {
			ascii_dual_relation(framed_group, outside_body, relation, AsciiRelationOptions{
				gap: max_int(display_width(relation) + display_width(crossing.label) + 4, 10)
				width: width
	...
	if crossing_edges.len == 2 {
		...
		if has_incoming && has_outgoing {
			left_nodes, left_edges := extract_outside_flow_component(outside_nodes, outside_edges, incoming.from,
				true)
			right_nodes, right_edges := extract_outside_flow_component(outside_nodes, outside_edges,
				outgoing.to, false)
			if left_nodes.len > 0 && right_nodes.len > 0 && left_edges.len + right_edges.len == outside_edges.len {
				...
				if left_body.split_into_lines().len == 1 && right_body.split_into_lines().len == 1 {
					...
					return ascii_triple_relation(left_body, framed_group, right_body, left_relation,
						right_relation, AsciiTripleRelationOptions{
						left_gap: max_int(display_width(left_relation) + display_width(incoming.label) + 4, 8)
						right_gap: max_int(display_width(right_relation) + display_width(outgoing.label) + 4, 8)
						width: width
		...
		if incoming_edges.len == 2 && outside_edges.len == 0 {
			if incoming_edges[0].label == incoming_edges[1].label && incoming_edges[0].kind == incoming_edges[1].kind && outside_nodes.len == 2 {
				sources := incoming_edges.map(flow_node_inline(it.from, flow_node_label_map(outside_nodes),
					flow_node_shape_map(outside_nodes)))
		...
		if incoming_edges.len == 2 && outside_edges.len > 0 {
			...
			for edge in incoming_edges {
				component_nodes, component_edges := extract_outside_flow_component(outside_nodes, outside_edges,
					edge.from, true)
			...
			if source_blocks.len == 2 && source_blocks.all(it.split_into_lines().len == 1) && covered_edges == outside_edges.len && incoming_edges[0].label == incoming_edges[1].label && incoming_edges[0].kind == incoming_edges[1].kind {
				left_body := ascii_lr_merge_to_gap(source_blocks, incoming_edges[0].label)
				return ascii_side_by_side_middle(left_body, framed_group, 2, width)
			}
		}
		if outgoing_edges.len == 2 && outside_edges.len == 0 {
			if outgoing_edges[0].label == outgoing_edges[1].label && outgoing_edges[0].kind == outgoing_edges[1].kind && outside_nodes.len == 2 {
				targets := outgoing_edges.map(flow_node_inline(it.to, flow_node_label_map(outside_nodes),
					flow_node_shape_map(outside_nodes)))
		...
		if outgoing_edges.len == 2 && outside_edges.len > 0 {
			...
			if target_blocks.len == 2 && target_blocks.all(it.split_into_lines().len == 1) && covered_edges == outside_edges.len && outgoing_edges[0].label == outgoing_edges[1].label && outgoing_edges[0].kind == outgoing_edges[1].kind {
		...
		if incoming_edges.len == 2 && outgoing_edges.len == 1 {
			...
			if source_blocks.len == 2 && source_blocks.all(it.split_into_lines().len == 1)
				...
				if right_body.split_into_lines().len == 1 {
					if graph.direction == .top_down {
						return render_ascii_td_group_merge_out(source_blocks, incoming_edges[0].label,
							framed_group, outgoing_edges[0].label, right_body, width)
					}
					left_body := ascii_lr_merge_to_gap(source_blocks, incoming_edges[0].label)
					right_relation := if outgoing_edges[0].kind == .arrow { '──▶' } else { '───' }
					return ascii_triple_relation(left_body, framed_group, right_body, '', right_relation,
						AsciiTripleRelationOptions{
						left_gap: 2
						right_gap: max_int(display_width(right_relation) + display_width(outgoing_edges[0].label) + 4, 8)
						width: width
		...
		if incoming_edges.len == 1 && outgoing_edges.len == 2 {
			...
			if left_nodes.len > 0 && target_blocks.len == 2
				...
				if left_body.split_into_lines().len == 1 {
					...
					right_body := ascii_lr_branch_from_gap(outgoing_edges[0].label, target_blocks)
					left_relation := if incoming_edges[0].kind == .arrow { '──▶' } else { '───' }
					return ascii_triple_relation(left_body, framed_group, right_body, left_relation, '',
						AsciiTripleRelationOptions{
						left_gap: max_int(display_width(left_relation) + display_width(incoming_edges[0].label) + 4, 8)
						right_gap: 2
						width: width
	...
	if crossing_edges.len == 4 {
		...
		if incoming_edges.len == 2 && outgoing_edges.len == 2 {
			...
			if outside_edges.len == 0 {
				source_blocks = incoming_edges.map(flow_node_inline(it.from, flow_node_label_map(outside_nodes),
					flow_node_shape_map(outside_nodes)))
				target_blocks = outgoing_edges.map(flow_node_inline(it.to, flow_node_label_map(outside_nodes),
					flow_node_shape_map(outside_nodes)))
			} else {
				for edge in incoming_edges {
			...
			if source_blocks.len == 2 && target_blocks.len == 2
				...
				&& outgoing_edges[0].kind == outgoing_edges[1].kind {
				if graph.direction == .top_down {
					return render_ascii_td_group_merge_branch(source_blocks, incoming_edges[0].label,
						framed_group, outgoing_edges[0].label, target_blocks, width)
				}
				left_body := ascii_lr_merge_to_gap(source_blocks, incoming_edges[0].label)
				right_body := ascii_lr_branch_from_gap(outgoing_edges[0].label, target_blocks)
...
fn render_ascii_td_group_merge_branch(sources []string, incoming_label string, framed_group string, outgoing_label string, targets []string, width int) string {
	...
	total_width := min_int(max_int(max_int(required_merge_width, frame_width), ascii_block_width(branch_block)), width)
...
fn render_ascii_td_group_merge_out(sources []string, incoming_label string, framed_group string, outgoing_label string, target string, width int) string {
	frame_width := ascii_block_width(framed_group)
	required_merge_width := max_source_display_width(sources) * 2 + 3
	total_width := min_int(max_int(max_int(required_merge_width, frame_width), display_width(target)),
		width)
	center_col := centered_block_mid_column(framed_group, total_width)
	merge_block := ascii_td_merge_to_column(sources, center_col, total_width)
	mut parts := []string{}
...
fn render_ascii_td_group_in_branch(source string, incoming_label string, framed_group string, outgoing_label string, targets []string, width int) string {
	frame_width := ascii_block_width(framed_group)
	branch_block := ascii_td_branch_from_center('', targets)
	total_width := min_int(max_int(max_int(display_width(source), frame_width), ascii_block_width(branch_block)),
		width)
	center_col := centered_block_mid_column(framed_group, total_width)
	mut parts := []string{}
...
fn ascii_td_branch_from_center(label string, targets []string) string {
	...
	mut lines := []string{}
	if label.len > 0 {
		width := max_int(display_width(label), max_int(display_width(targets[0]), display_width(targets[1])) + 4)
...
fn render_flow_graph_component(graph DiagramGraph, width int) string {
	...
	mut blocks := []string{}
	for node in graph.nodes {
		blocks << flow_node_block(node.id, flow_node_label_map(graph.nodes), flow_node_shape_map(graph.nodes),
			max_int(width / max_int(graph.nodes.len, 1), 16))
	}
	return blocks.join('\n\n')
}
...
fn render_ascii_grouped_graph(edges []AsciiGraphEdge, width int, title string) string {
	...
	for from in order {
		...
		if next_targets.len == 1 {
			...
			rendered = rendered.replace('┴ ─▶ ' + sink_inline, '┴ ─▶ ' + sink_inline + ' ─▶ ' + next_inline)
...
fn render_ascii_flow_graph_lr(graph DiagramGraph, labels map[string]string, shapes map[string]DiagramNodeShape, width int) string {
	...
	for from in order {
		...
		for edge in merge_edges {
			if edge.label != merge_label || edge.kind != merge_kind || edge.from !in edges.map(it.to) {
				eligible = false
				break
			}
		}
...
fn render_ascii_flow_graph_td(graph DiagramGraph, labels map[string]string, shapes map[string]DiagramNodeShape, width int) string {
	if chain := flow_linear_chain(graph) {
		...
		for i, node_id in chain.nodes {
			...
			parts << block
			if i < chain.edges.len {
				for line in ascii_vertical_edge(ascii_block_width(block), chain.edges[i].kind == .arrow, chain.edges[i].label) {
					parts << line
				}
			}
		}
	...
	for i, from in order {
		...
		if edges.len == 1 {
			...
			for line in ascii_vertical_edge(ascii_block_width(source), edge.kind == .arrow, edge.label) {
				parts << line
			}
			parts << target
			lines << parts.join('\n')
