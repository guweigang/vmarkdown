...
pub fn (diagram MermaidDiagram) render_ascii(width int) string {
	if diagram.kind in [.sequence, .state, .class, .er, .gantt, .mindmap, .journey, .git_graph,
		.timeline] {
		if payload := diagram.to_diagram_payload() {
			return render_diagram_payload(payload, max_int(width, if diagram.kind in [
				.class,
				.er,
				.gantt,
			] {
	...
	for subgraph in diagram.subgraphs {
		if subgraph.title in rendered_subgraphs {
			parts << render_mermaid_subgraph_box(subgraph.title,
				rendered_subgraphs[subgraph.title], safe_width)
		}
	}
	return parts.join('\n\n')
}
...
fn (diagram MermaidDiagram) can_render_flow_via_diagram_payload() bool {
	...
	if diagram.subgraphs.len == 1 {
		...
		if crossing_edges == 3 {
			if !(incoming_crossing == 2 && outgoing_crossing == 1) && !(incoming_crossing == 1
				&& outgoing_crossing == 2) {
				return false
			}
	...
	if diagram.direction == .top_down {
		if diagram.paths.len == 1 {
			...
			return path.nodes.len == diagram.edges.len + 1
				&& path.edge_kinds.len == diagram.edges.len
		}
		return diagram.paths.len > 0 && diagram.paths.all(it.nodes.len == 2
			&& it.edge_kinds.len == 1)
	}
	if diagram.paths.len == 1 {
		path := diagram.paths[0]
...
fn (diagram MermaidDiagram) collect_td_branch_group(start int) ?MermaidBranchGroup {
	...
	for i := start; i < diagram.paths.len; i++ {
		...
		if path.nodes[0] != base.nodes[0] || path.subgraph != base.subgraph
			|| path.edge_kinds[0] != base.edge_kinds[0] {
			continue
		}
...
fn (diagram MermaidDiagram) collect_td_branch_merge_group(start int) ?MermaidBranchMergeGroup {
	...
	for i, path in diagram.paths {
		...
		if path.nodes[1] != merge_target_id || path.edge_kinds[0] != merge_kind
			|| label != merge_label {
			return none
		}
...
fn (diagram MermaidDiagram) collect_td_merge_group(start int) ?MermaidMergeGroup {
	...
	for i := start; i < diagram.paths.len; i++ {
		...
		if path.nodes[1] != base.nodes[1] || path.subgraph != base.subgraph
			|| path.edge_kinds[0] != base.edge_kinds[0] {
			continue
		}
		label := if path.edge_labels.len > 0 { path.edge_labels[0] } else { '' }
...
fn (diagram MermaidDiagram) render_td_path(path MermaidPath) string {
	...
	for i, node_id in path.nodes {
		...
		if i < path.edge_kinds.len {
			...
			lines << ascii_vertical_edge(ascii_block_width(node_lines[0]),
				path.edge_kinds[i] == .arrow, label)
		}
	}
