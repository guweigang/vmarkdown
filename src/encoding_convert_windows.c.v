...
fn decode_text_encoding(bytes []u8, encoding MarkdownEncoding) !string {
	return match encoding {
		...
		.utf8 { return error('UTF-8 does not require platform conversion') }
	}
}
...
fn windows_decode_code_page(bytes []u8, code_page u32, label string) !string {
	...
	wide_len := C.MultiByteToWideChar(code_page, windows_mb_err_invalid_chars, bytes.data,
		bytes.len, 0, 0)
	if wide_len <= 0 {
		return error('file is not valid ${label}')
	}
	mut wide := []u8{len: wide_len * 2}
	if C.MultiByteToWideChar(code_page, windows_mb_err_invalid_chars, bytes.data, bytes.len,
		wide.data, wide_len) != wide_len {
		return error('file is not valid ${label}')
	}
...
fn windows_encode_code_page(text string, code_page u32, label string) ![]u8 {
	...
	encoded_len := C.WideCharToMultiByte(code_page, 0, wide.data, wide.len / 2, 0, 0, 0,
		&used_default)
...
fn windows_utf8_to_wide(text string) ![]u8 {
	...
	wide_len := C.MultiByteToWideChar(windows_cp_utf8, windows_mb_err_invalid_chars, text.str,
		text.len, 0, 0)
	if wide_len <= 0 {
		return error('text is not valid UTF-8')
	}
	mut wide := []u8{len: wide_len * 2}
	if C.MultiByteToWideChar(windows_cp_utf8, windows_mb_err_invalid_chars, text.str, text.len,
		wide.data, wide_len) != wide_len {
		return error('text is not valid UTF-8')
	}
	return wide
...
fn windows_wide_to_utf8(wide []u8, label string) !string {
	...
	utf8_len := C.WideCharToMultiByte(windows_cp_utf8, windows_wc_err_invalid_chars, wide.data,
		wide.len / 2, 0, 0, 0, 0)
	if utf8_len <= 0 {
		return error('file is not valid ${label}')
	}
	mut utf8 := []u8{len: utf8_len}
	if C.WideCharToMultiByte(windows_cp_utf8, windows_wc_err_invalid_chars, wide.data,
		wide.len / 2, utf8.data, utf8.len, 0, 0) != utf8_len {
		return error('file is not valid ${label}')
	}
	return utf8.bytestr()
}
...
fn decode_utf32(bytes []u8, little_endian bool) !string {
	...
	for i := 0; i < bytes.len; i += 4 {
		value := if little_endian {
			...
			(u32(bytes[i]) << 24) | (u32(bytes[i + 1]) << 16) | (u32(bytes[i + 2]) << 8) | u32(bytes[
				i + 3])
		}
