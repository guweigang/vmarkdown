...
fn test_ast_source_blocks_use_parser_spans_and_expand_fences() {
	markdown := '# Título\n\npara\ncontinues\n\n---\n\n```v\nprintln(1)\n```\n'
	...
	assert blocks[1].end_line == 3
	assert blocks[2].start_line == 5
	assert blocks[3].start_line == 7
	assert blocks[3].end_line == 9
}
...
fn test_preview_column_map_skips_render_only_markup() {
	columns, exact := build_preview_source_columns('# **Hello** world', '<h1><strong>Hello</strong> world</h1>', .html)
...
fn test_preview_column_map_advances_across_wrapped_repeated_words() {
	...
	first_columns, first_exact := build_preview_source_columns_from(source, 'word alpha', .terminal, 0)
	...
	second_columns, second_exact := build_preview_source_columns_from(source, 'word beta', .terminal, next_source_column)
