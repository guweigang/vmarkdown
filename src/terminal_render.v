...
fn (r TerminalRenderer) render_heading(text string, level int) string {
	match level {
		1 {
			...
			rule :=
				r.style_line('━'.repeat(min_int(max_int(text.len, 12), min_int(r.width, 40))), TerminalStyle{'heading1_rule'})
			return '${title}\n${rule}'
		}
		2 {
			title := r.style_line(text, TerminalStyle{'heading2'})
			rule :=
				r.style_line('─'.repeat(min_int(max_int(text.len, 10), min_int(r.width, 32))), TerminalStyle{'heading2_rule'})
			return '${title}\n${rule}'
		}
		else {
...
fn (r TerminalRenderer) render_list_item(item ListItemNode, depth int) []string {
	mut out := []string{}
...
fn merge_terminal_spaces(spans []TerminalSpan) []TerminalSpan {
	...
	for span in spans {
		...
		if merged.len > 0 && is_space_only(merged[merged.len - 1].plain)
			&& is_space_only(span.plain) {
			continue
		}
		merged << span
	}
...
fn wrap_terminal_lines(spans []TerminalSpan, width int) []string {
	...
	for span in spans {
		...
		if current_plain > 0 && current_plain + token_plain.len > width
			&& !is_space_only(token_plain) {
			lines << current.str().trim_right(' ')
			current = strings.new_builder(128)
			current_plain = 0
		}
