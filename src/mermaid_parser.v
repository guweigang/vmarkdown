...
fn parse_mermaid_header(line string, diagram MermaidDiagram) !(MermaidDiagram, bool) {
	...
	if kind !in ['flowchart', 'graph', 'sequencediagram', 'statediagram-v2', 'classdiagram',
		'erdiagram', 'gantt', 'mindmap', 'journey', 'gitgraph', 'timeline'] {
		return diagram, false
	}
...
fn parse_mermaid_sequence_note(line string) ?MermaidSequenceNote {
	...
	if parts.len < 3
		|| (parts[0].to_lower() != 'left' && parts[0].to_lower() != 'right')
		|| parts[1].to_lower() != 'of' {
		return none
	}
...
fn parse_mermaid_relation_line(line string, mut diagram MermaidDiagram, mut seen map[string]int, current_subgraph string) ! {
	...
	if specs.len == 2 && specs[1].contains('&') {
		parse_mermaid_branch_relation(specs[0], specs[1], edge_kinds[0], edge_labels[0], mut
			diagram, mut seen, current_subgraph)!
		return
	}
