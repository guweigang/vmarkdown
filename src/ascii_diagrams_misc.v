...
pub fn render_ascii_timeline_diagram(timeline DiagramTimeline, width int) string {
	...
	if timeline.title.len > 0 {
		...
		lines << truncate_display_width('─'.repeat(min_int(max_int(display_width(timeline.title), 8),
			width)), width)
		lines << ''
	}
	...
	for event in sequence.events {
		match event {
			DiagramSequenceMessage {
				lines << render_ascii_sequence_message(sequence.participants, event, lane_width,
					width)
			...
			DiagramSequenceBlockBoundary {
				lines << render_ascii_sequence_block_boundary(event, lane_width,
					sequence.participants.len, width)
				lines << ascii_lifelines(sequence.participants, lane_width, active)
			}
		}
	}
...
pub fn render_ascii_journey(journey DiagramJourney, width int) string {
	...
	if journey.title.len > 0 {
		...
		lines << truncate_display_width('─'.repeat(min_int(max_int(display_width(journey.title), 8),
			width)), width)
		lines << ''
	}
